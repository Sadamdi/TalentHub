...
router.get('/:id', async (req, res) => {
  try {
    const job = await Job.findById(req.params.id)
      .populate('companyId', 'companyName logo industry description website');
...
router.post('/', [
  ...
  body('description').notEmpty().withMessage('Deskripsi pekerjaan diperlukan'),
  body('salary.amount').isNumeric().withMessage('Gaji harus berupa angka'),
  body('category').isIn(['designer', 'writer', 'finance', 'developer', 'marketing', 'sales', 'other']).withMessage('Kategori tidak valid'),
  body('location').notEmpty().withMessage('Lokasi pekerjaan diperlukan'),
  body('experienceLevel').isIn(['fresh_graduate', '1-2_years', '3-5_years', '5+_years']).withMessage('Level pengalaman tidak valid'),
  body('jobType').isIn(['full_time', 'part_time', 'contract', 'internship', 'freelance']).withMessage('Tipe pekerjaan tidak valid')
], async (req, res) => {
  try {
...
router.put('/:id', [
  ...
  try {
    ...
    const allowedFields = [
      ...
      'salary', 'location', 'jobType', 'experienceLevel', 'skills',
      'benefits', 'applicationDeadline', 'isActive', 'category'
    ];
