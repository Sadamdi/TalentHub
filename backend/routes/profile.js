...
router.post(
	...
	[
		auth,
		requireRole(['company']),
		body('companyName').notEmpty().withMessage('Nama perusahaan diperlukan'),
		body('industry').notEmpty().withMessage('Industri diperlukan'),
		body('description')
			.optional()
			.isLength({ max: 1000 })
	...
	async (req, res) => {
		try {
			...
			await company.populate('userId', 'email createdAt');
...
router.post(
	...
	[
		...
		body('mediaType')
			...
			.withMessage('Tipe media tidak valid'),
	],
	async (req, res) => {
		try {
			const errors = validationResult(req);
...
router.delete(
	...
	async (req, res) => {
		try {
			...
			talent.portfolio = talent.portfolio.filter(
